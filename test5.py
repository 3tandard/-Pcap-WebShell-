        ...
        try:
            ...
            if method == "POST" and post_data:
                ...
                for pattern in WEBSHELL_PATTERNS:
                    if re.search(re.escape(pattern), decoded_data):  # 使用 re.escape() 进行转义匹配
                        detected_patterns.append(pattern)

                print(f"POST Data (ASCII): {decoded_data}")

                # 如果检测到 WebShell 相关特征，则报警
                if detected_patterns:
                    print(f"[⚠] Detected potential WebShell payload! Matches: {', '.join(detected_patterns)}")
